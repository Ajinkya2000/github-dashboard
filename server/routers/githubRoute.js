...
router.post("/repos", async (req, res) => {
  try {
    const params = { sort: 'created', direction: req.body.direction, per_page: req.body.limit };
    ...
    if (response.headers.link != undefined) {
      const { nextPage, lastPage } = getPageNumbers(
        String(response.headers.link)
      );

      res.status(200).send({ data: response.data, nextPage, lastPage });
    } else {
      res
        .status(200)
        .send({ data: response.data, nextPage: null, lastPage: null });
    }
  } catch (error) {
